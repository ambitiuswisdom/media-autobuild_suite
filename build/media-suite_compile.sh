...
if { [[ $other265 = y ]] || { [[ $ffmpeg != no ]] && enabled libkvazaar; }; } &&
    ...
    grep_or_sed 'KVZ_STATIC_LIB' "$LOCALDESTDIR/lib/pkgconfig/kvazaar.pc" \
    '/^Cflags:/a Cflags.private: -DKVZ_STATIC_LIB'
    do_checkIfExist
fi
...
_check=(wels/codec_{api,ver}.h libopenh264.a openh264.pc)
if [[ $ffmpeg != no ]] && enabled libopenh264 &&
    do_vcs "$SOURCE_REPO_OPENH264"; then
    do_uninstall "${_check[@]}"
    do_mesoninstall video -Dtests=disabled
    do_checkIfExist
fi
